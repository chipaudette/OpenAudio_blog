...
AudioConnection patchCord3(myEffect, 0, usb_out, 0);
...
void loop(void)
{
  ...
  for (int Iloop=0; Iloop < 5; Iloop++) {
    ...
    if(!myEffect.play(t_ampx,t_lox,t_hix,t_timex)) {
      ...
      Serial.print("ToneSweep_Tympan: starting sweep, amplitude ");
    ...
    while(myEffect.isPlaying()) {
      if ((millis() - prevMillis) > 1000) {
        Serial.print("RMS (dB) = ");Serial.println(20.*log10f(rms.read()));
        prevMillis = millis();
      }
    }
