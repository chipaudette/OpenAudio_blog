...
params.n_cycles = 5;
params.drive_amplitude = drive_amplitude; %looking at the USB stream, they're only have the size of the given values
...
%analyze data
[out]=f_measureSweeps2(wav_Pa,fs_Hz,t_start_sec,params);
...
if (0)
    ...
    plot(out.t_win_sec,20*log10(out.wav_win_rms/norm_fac));
end
xlim(xl);
xlabel('Time (sec)');
switch data_type
...
if (0)
    ...
    for Isweep=1:params.n_cycles
        I=find(out.win_sweep_id == Isweep);
        semilogx(out.f_win_Hz(I),20*log10(out.wav_win_rms(I)/norm_fac),'linewidth',2);
        hold on;
    end
end
xlabel('Frequency (Hz)');
